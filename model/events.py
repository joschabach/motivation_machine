...
def change_event(id, expected_reward = None, certainty = None, skill = None, expiration = None):
    ...
    if expiration is not None: event.expiration = expiration

    event_relevance = abs(event.consumption.get_anticipated_reward(event.expected_reward, event.expiration)
                          ) * event.consumption.need.weight

    # trigger a satisfaction or frustration event based on the anticipated reward change
    event.consumption.anticipate(factor * expected_reward_delta, event.certainty, event.skill, event.expiration)

    # react to changes in certainty
    if certainty_delta > 0: # increase in certainty, proportional to relevance of event
        confirmation.trigger(certainty_delta * event_relevance)
    if certainty_delta < 0:  # decrease in certainty
        disconfirmation.trigger(- certainty_delta * event_relevance)

    # react to changes in expected competence
    if skill_delta > 0:  # increase in epistemic competence
        success.trigger(skill_delta * event_relevance)
    if skill_delta < 0:
        failure.trigger(-skill_delta * event_relevance)
...
def execute_event(id, reward = None):
    ...
    if reward<0:  # aversive event
        if event.expected_reward <0:
            confirmation.trigger((1-abs(reward-event.expected_reward))*relevance)  # I knew it!
            if event is goal:
                failure.trigger(event.skill*relevance*(1-event.certainty))  # I failed at my skillz
            failure.trigger(-relevance)
        else:  # this is an aversive event, so the best case is 0 reward...
            disconfirmation.trigger(-reward*relevance)  # better than I thought
            success.trigger(relevance)  # I have avoided a bad outcome
    else:  # appetitive event
        confirmation.trigger((1 - abs(reward - event.expected_reward)) * relevance)  # I knew it!
        if event is goal:
            success.trigger((1-event.skill) * relevance * (1-event.certainty))  # I succeeded at my skillz
        success.trigger(relevance)

    if event is goal: set_goal("default")
    remove_event(id)

def consume(consumption_name, reward = None):
    """Just consume an unexpected gain or loss"""
...
def reset():
    global goal, events
    events = {}
    create_event("default", "recover", certainty = 0, skill = 1.0, expiration = -1)
    set_goal("default")
